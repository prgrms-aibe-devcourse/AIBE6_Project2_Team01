...
// accessToken이 만료(401)되면 refreshToken으로 재발급 후 원요청을 한 번만 재시도한다.
// 아래 경로들은 인증이 필요 없거나(로그인/가입/이메일 인증) 재발급 대상이 아니므로(로그아웃/재발급 자체) 제외한다.
...
/** refreshToken까지 만료되어 재발급이 실패했을 때 호출할 콜백을 등록한다. AuthProvider가 mount 시 등록한다. */
export function setSessionExpiredHandler(handler: (() => void) | null) {
  sessionExpiredHandler = handler
}

// onRequest 시점에 요청 바디가 소비되기 전에 복제해두고, onResponse에서 재시도용으로 사용한다.
