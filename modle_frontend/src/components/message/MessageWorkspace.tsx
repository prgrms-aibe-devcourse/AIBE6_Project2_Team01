...
function buildThreads(
  ...
  return [...inbox.conversations]
    ...
    .map((conversation) => {
      const participantId =
        conversation.clientId === inbox.currentUser.id
      ...
      return {
        ...
        preview: last?.content ?? "메시지가 없습니다.",
        time: last
...
export function MessageWorkspace() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const recipientId = Number(params.get("recipientId"));
      const draft =
      ...
      try {
        ...
        setThreads((currentThreads) => {
          ...
          if (draft && !existingTarget) {
            loadedThreads.unshift(draft);
          }
  ...
  useEffect(() => {
    ...
    void markConversationAsRead(selectedThread.conversationId)
      .then(() => {
        setThreads((current) =>
          current.map((thread) =>
    ...
    <main className="relative h-[calc(100dvh-64px)] overflow-hidden bg-canvas-soft p-0 md:p-2">
      ...
      <div className="mx-auto grid h-full max-w-[1360px] overflow-hidden border-hairline bg-surface shadow-sm md:grid-cols-[270px_minmax(0,1fr)] md:rounded-xl md:border xl:grid-cols-[270px_minmax(0,1fr)_240px]">
              ...
              visibleThreads.map((thread) => {
                ...
                const swipeOffset = isSwiping
                  ...
                  : swipedThreadId === thread.id
                    ...
                    : 0;
                const isDeleting = deletingConversationId === thread.conversationId;
        ...
        <section className="flex min-h-0 min-w-0 flex-col bg-surface">
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-xs text-mute">
                  ...
                  {selectedThread.conversationId ? "대화 중" : "새 제안"}
                </p>
              </div>
            </div>
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto bg-canvas-soft px-4 py-4 md:px-6">
            {selectedThread.messages.length === 0 ? (
              <div className="grid flex-1 place-items-center text-center">
                <div>
                  <div className="mx-auto grid size-14 place-items-center rounded-full border border-hairline bg-white text-xl shadow-sm">
                    ✉️
                  </div>
                  <p className="mt-4 font-semibold text-ink">
                    첫 메시지를 보내 대화를 시작해 주세요.
                  </p>
                  <p className="mt-1 text-xs text-mute">
                    공고를 연결하거나 일반 메시지로 제안을 보낼 수 있습니다.
                  </p>
                </div>
              </div>
            ) : (
              selectedThread.messages.map((message: MessageItem, index) => {
                ...
                return (
                  <div className="contents" key={message.id}>
                    {showDate && (
                      <div className="mx-auto rounded-full border border-hairline bg-white px-3 py-1 text-[11px] font-medium text-mute">
                        {new Intl.DateTimeFormat("ko-KR", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                    ...
                    <article
                      className={`group flex max-w-[82%] flex-col md:max-w-[68%] ${
          ...
          <form
            className="shrink-0 border-t border-hairline bg-white px-4 py-2.5"
          ...
          >
            <div className="flex items-end gap-2 rounded-xl border border-hairline bg-canvas-soft p-1.5 transition focus-within:border-hairline-strong focus-within:bg-white">
              <textarea
                className="max-h-24 min-h-9 flex-1 resize-none bg-transparent px-2 py-1.5 text-sm leading-5 outline-none"
              ...
              />
              <button
                className="grid size-9 shrink-0 place-items-center rounded-lg bg-ink text-sm font-bold text-white transition hover:bg-primary-hover disabled:cursor-not-allowed disabled:opacity-25"
        ...
        <aside className="hidden min-h-0 overflow-y-auto border-l border-hairline bg-canvas px-4 py-5 xl:block">
          ...
          <div className="mt-4 flex flex-col items-center border-b border-hairline pb-5 text-center">
            {selectedThread.participantProfileImageUrl ? (
              ...
              <div className="grid size-14 place-items-center rounded-full bg-canvas-soft text-lg font-bold text-body">
                {participantInitial(selectedThread.participantName)}
              </div>
            )}
          ...
          <div className="mt-5">
            ...
            <div className="mt-2.5 rounded-lg border border-hairline bg-white p-3.5">
              <span className="inline-flex rounded-full bg-canvas-soft px-2 py-1 text-[10px] font-bold text-body">
                {selectedThread.postId ? "공고 제안" : "일반 채팅"}
              </span>
              <h4 className="mt-3 text-sm font-bold leading-5 text-ink">
                {selectedJob?.title ??
